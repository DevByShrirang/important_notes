...
Q- jenkins pipeline failed during deployment stage. what is the troubleshooting steps.
First i will review pipeline console output to indentify any specific error and failre message.

1) pipeline console output- 
2) identify specific error or failure messages- 
3) review pipeline config.--
4) build steps , env variables , and plugins version.
5) consult logs from application, docker container, or external services.
6) involved in deployment process for further insights.

answer to interviewver -- docker build was failing due to missing file during the copy instruction.
i then check git repository and realized the developer had renamed a file(config.dev.json) but did not update the dockeffile
i fix the dockerfile to match new filename
after that i manually ran build and verfied that the container was created successfully.

additionally i added sonarqube rule and pre-commit hook to check for dockerfile issues in other services by introducing a pre-build validation script.
